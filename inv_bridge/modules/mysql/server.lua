...
local Query = {
    ...
    SELECT_PLAYER = 'SELECT inventory FROM `{user_table}` WHERE `{user_column}` = ? AND charIdentifier = ?',
    ...
    UPDATE_PLAYER = 'UPDATE `{user_table}` SET inventory = ? WHERE `{user_column}` = ? AND charIdentifier = ?',
}
...
Citizen.CreateThreadNow(function()
    ...
    elseif shared.framework == 'nd' then
        ...
        vehicleColumn = 'id'
    elseif shared.framework == 'LEGACYCORE' then
        playerTable = 'users'
        playerColumn = 'identifier'
        vehicleTable = 'owned_vehicles'
        vehicleColumn = 'plate'
...
function db.loadPlayer(identifier, slot)
    local inventory = MySQL.prepare.await(Query.SELECT_PLAYER, { identifier , slot }) --[[@as string?]]
    return inventory and json.decode(inventory)
end

function db.savePlayer(owner, inventory, slot)
    return MySQL.prepare(Query.UPDATE_PLAYER, { inventory, owner, slot })
end
