...
function Utils:CameraMultichar(slot, datacoords, spawnConfig)
    ...
    local camCoords = datacoords[currentSlot]?.camPos or
        vector3(entityCoords.x, entityCoords.y, entityCoords.z + (start and 0.0 or 0.90))
...
function Utils.requestModel(model)
    local LoadedModel = lib.requestModel(model)
    return LoadedModel
end

function Utils:SetFreemodeModel(sex)
    local model = sex == "m" and `mp_m_freemode_01` or `mp_f_freemode_01`
    local modelToSet = Utils.requestModel(model)
    SetPlayerModel(PlayerId(), modelToSet)
    SetModelAsNoLongerNeeded(modelToSet)
